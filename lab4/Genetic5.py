...
class Genetic5:
    ...
    def calc_fitness(self, population: list[Struct]):
        for test in self.parasites:
            ...
            for network in self.population:
              if sortByNetwork(network.arr,test.arr):
                  fitness+=1
        ...
        for network in self.population:
            ...
            for test in self.parasites:
                if sortByNetwork(network.arr,test.arr):
                    fitness+=1
            network.fitness=(fitness/self.args.GA_POPSIZE)*network.arr.len()


    def sortByNetwork(self,network,test):

        for i in range(0,network.len(),2):
            temp=test[i+1]
            test[i+1]=test[i]
            test[i]=temp

        for i in range(test.len()-1):
            if(test[i]>test[i+1]):
                return 1
    ...
    def mate(self, population: list[Struct], buffer: list[Struct]):
        ...
        for i in range(self.esize, self.args.GA_POPSIZE):
            ...
            gene =[]
            for j in range(0,min(population[i1].arr,population[i2].arr),2):
                # if self.population[i1].str[j-1]<self.tsize:
                if(j%4==0):
                   population[i1].arr[j]=population[i2].arr[j]
                   population[i1].arr[j+1] = population[i2].arr[j+1]
